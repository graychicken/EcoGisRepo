...
class xCSS
{
	...
	public function compile($input_xcss = FALSE)
	{
		if($input_xcss === FALSE)
		{
			...
			for($i = 0; $i < $count_xcss_files; $i++)
			{
				...
				$filename = $this->xcss_files[$i][0] == '/' ? $this->xcss_files[$i] : $this->path_css_dir.$this->xcss_files[$i];
			...
			if( ! empty($this->final_file))
			{
				if($this->compress_output_to_master)
				{
					...
					{
						$fname = explode(':', $fname);
						$master_content .= $this->read_file($fname[0][0] == '/' ? $fname[0] : $this->path_css_dir.$fname[0])."\n";
					}
					...
					{
						$fname = explode(':', $fname);
						$tmp_file = $this->read_file($fname[0][0] == '/' ? $fname[0] : $this->path_css_dir.$fname[0]);
	...
	public function create_file($content, $filename)
	{
		...
		$filepath = $filename[0] == '/' ? $filename : $this->path_css_dir.$filename;
		if( ! file_exists($filepath))
		{
